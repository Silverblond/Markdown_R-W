...
2. 처음 실행 시 **"파일이 손상되었습니다"** 또는 "확인되지 않은 개발자" 경고가 뜨면 터미널에서 아래 명령어를 실행하세요:
   ```bash
   xattr -cr "/Applications/Markdown Viewer.app"
   ```
   이후 앱을 다시 실행하면 정상적으로 열립니다.

> **원인**: 인터넷에서 다운받은 파일에 macOS가 격리(quarantine) 속성을 자동으로 붙이는데, Apple 인증서로 서명·공증되지 않은 앱은 "손상된 파일"로 처리됩니다. `xattr -cr`로 해당 속성을 제거하면 해결됩니다.
